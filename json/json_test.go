...
func TestCSVFileAdapter_Read(t *testing.T) {
	...
	jsonAdapter, err := New[TestUser]([]byte(`
	[
		{ "name": "jacek", "age": 26, "CreatedAt": "2012-04-01T15:00:00Z" },
		{ "name": "john", "CreatedAt": "0001-01-01T00:00:00Z" }
	]`))
	if err != nil {
		t.Fatal(err)
	}
	...
	records, err := jsonAdapter.Read(ctx)
	if err != nil {
		t.Fatal(err)
	}
	...
	originalRecords, err := jsonAdapter.Read(ctx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := jsonAdapter.Upsert(ctx, processedRecords); err != nil {
	...
	updatedRecords, err := jsonAdapter.Read(ctx)
	if err != nil {
		t.Fatal(err)
	}
